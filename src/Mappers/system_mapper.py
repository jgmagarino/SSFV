...
def get_all_systems() -> list[System]:
    ...
    for i in range(len(result)):
        name, id_panel, place, progress, description = result[i]
        new_system = System(name, id_panel, place, progress)
...
def get_system(name) -> System:
    ...
    name, panel_id, place, progress, description = result
    system = System(name, panel_id, place, progress)
    return system
